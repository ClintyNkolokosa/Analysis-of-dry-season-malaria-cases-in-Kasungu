...
suppressPackageStartupMessages({
  ...
  library(insight)       # gain insight into statistical model (useful model info and data)
  library(ggpubr)
  library(plotly)
  library(lubridate)
  library(knitr)
...
# See Kasungu_malaria.RMD on how the buffers were generated
...
buffer_3km_2020 <- sf::st_read(here::here("data", "buffer_3km_2020.shp"))
...
A ratio greater than 1.0 indicates that more malaria cases have occurred than would have been expected, while a ratio less than 1.0 indicates that less cases have occurred. Thus, catchments with SMRs above 1 have high dry season malaria risk.
...
```{r, message=FALSE, warning=FALSE, fig.height=15, fig.width=11, fig.cap='Fig 11. Relationship between standardised morbidity ratio and living near waterbodies. A correlation coefficient of more than zero (cor.coeff *r* \> 0.1) indicates some positive association between the SMR and the buffer population variables. That is, SMR of dry season malaria increases with increase in number of people surrounding water bodies. This implies that as the population of people living near water bodies increases, the risk of dry season malaria increases as well.'}
...
Mathematically, our multivariate model can be written as: $$ln (E(y)) = 𝜷_0 + {𝜷_1} {x_i}_{1} + ln(𝒆_𝒊)$$ 
...
insight::model_info(multivariate_1km)
...
Here we apply a `quassi-Poisson` regression model to the multivariate model, which we have observed that it suffers from overdispersion issues under regular `Poisson` regression. One approach to dealing with overdispersion is to model the response using a `quassi-Poisson` instead of a `Poisson` distribution. An advantage of this approach is that it introduces another parameter in addition to $λ$, which gives the model more flexibility. 
...
## Scatter plot of fitted vs obeserved values

```{r, message=FALSE, fig.height=5, fig.width=12, fig.cap='Fig. 3. How well the percentage of catchment population living around water bodies explain observed malaria incidence'}
...
# Invoking function ------------------------------------------------------------
scatter_plot_1km <- plot.fitted.values(multivariate_1km, model_data, title = "1km model")

scatter_plot_2km <- plot.fitted.values(multivariate_2km, model_data, title = "2km model")

scatter_plot_3km <- plot.fitted.values(multivariate_3km, model_data, title = "3km model")

# Layout scatter plots ---------------------------------------------------------
cowplot::plot_grid(scatter_plot_1km, scatter_plot_2km, scatter_plot_3km, ncol = 3)
...
3. Use the model to predict the response value of the one observation left out of the model and calculate the ` root mean squared error` (`RMSE`).

4. Repeat this process *n* times. Calculate the test `RMSE` to be the average of all of the test `RMSE`’s.

Two important metrics have been used to assess the performance of the predictive regression model:

`RMSE`: The `Root Mean Squared Error`, which measures the model prediction error. It corresponds to the average difference between the observed known values of the outcome and the predicted value by the model.`RMSE` is computed as `RMSE = mean((observed - predicted)^2) |> sqrt()`. The lower the `RMSE`, the more closely a model can predict the actual observations. That is, the closer the model predictions are to the observations, the smaller the `RMSE` will be. Thus, the lower the `RMSE`, the better the model. 
...
train_multivariate_model_1km <- caret::train(
  observed_cases~1+prop_pop_1km+year+offset(log(expected_cases)),
  data = model_data, method = "glm", 
  trControl = train_control, family = "poisson")

train_multivariate_model_2km <- caret::train(
  observed_cases~1+prop_pop_2km+year+offset(log(expected_cases)),
  data = model_data, method = "glm", 
  trControl = train_control, family = "poisson")


train_multivariate_model_3km <- caret::train(
  observed_cases~1+prop_pop_3km+year+offset(log(expected_cases)),
  data = model_data, method = "glm", 
  trControl = train_control, family = "poisson")

# Summarize the results
# The lower the RMSE and MAE the more closely a model can predict the actual observations
print(train_multivariate_model_1km)

print(train_multivariate_model_2km)

print(train_multivariate_model_3km)

# Compare model performance
# performance::compare_performance(multivariate_1km, multivariate_2km, multivariate_3km,
...
104 different samples were used to build 104 models. Each model used 3 predictor variables.

No pre-processing occurred. That is, we didn’t scale the data in any way before fitting the models.
    
The resampling method we used to generate the 104 samples was Leave-One-Out Cross Validation.
    
The sample size for each training set was 103. 
