...
Monthly variation in malaria cases
...
# observed malaria cases = -0.2263102 + 0.0247164 * percentage of catchment population living near dam 
...
# Prediction
predicted_2017 <- predict(model_2017, test_2017)
...
```{r, message=FALSE, fig.height=15, fig.width=11, fig.cap='Fig. 12. How well the percentage of catchment population living around water bodies explain observed malaria incidence'}
...
# Prep data
spatial_dependency_data <- model_data_2017 |>
...
catchment_neighbours <- spdep::poly2nb(spatial_dependency_data)  # Queen contiguity

summary(catchment_neighbours)

# Get coordinates from catchment polygons
catchment_points <- as(spatial_dependency_data, "Spatial")
...
catchment_polygons <- as(spatial_dependency_data, "Spatial")

# View the connections
{plot(catchment_polygons, asp = 1)+
plot(catchment_neighbours, coords, col = "blue", add = TRUE)}

# Run a Moran I test on observed malaria cases
moran.test(spatial_dependency_data$SMR_2017, 
           nb2listw(catchment_neighbours))

moran.test(spatial_dependency_data$observed_2018, 
           nb2listw(catchment_neighbours))

moran.test(spatial_dependency_data$observed_2019, 
           nb2listw(catchment_neighbours))

moran.test(spatial_dependency_data$observed_2020, 
           nb2listw(catchment_neighbours))

# Run a Moran I MC test on observed malaria cases
moran.mc(spatial_dependency_data$observed_2017, 
         nb2listw(catchment_neighbours), 
         nsim = 999)

moran.mc(spatial_dependency_data$observed_2018, 
         nb2listw(catchment_neighbours), 
         nsim = 999)

moran.mc(spatial_dependency_data$observed_2019, 
         nb2listw(catchment_neighbours), 
         nsim = 999)

moran.mc(spatial_dependency_data$observed_2020, 
         nb2listw(catchment_neighbours), 
         nsim = 999)
...
Let's use `autoKrige()` function in the `automap` package to compute binned variograms, fit Kriging model, do model selection, and performs kriging by making multiple calls to the `gstat` functions.
...
# Custom function to merge and tidy model data ---------------------------------
merge.model.data <- function(data_2017, data_2018, data_2019, data_2020){
  
  tibble_2017 <- data_2017 |> 
    dplyr::as_tibble() |> 
    dplyr::rename(prop_pop_1km_2017 = prop_pop_1km,
                  prop_pop_2km_2017 = prop_pop_2km) |> 
    dplyr::select(-geometry, -prop_pop_3km, -fid, -DN, -X,
                  -dr_2018, -dr_2019, -dr_2020, -SMR)
                  
  tibble_2018 <- data_2018 |> 
    dplyr::as_tibble() |>
    dplyr::rename(prop_pop_1km_2018 = prop_pop_1km,
                  prop_pop_2km_2018 = prop_pop_2km) |> 
    dplyr::select(-c(Names, geometry, prop_pop_3km, fid,
                     DN, X, dr_2017, dr_2019, dr_2020, SMR))
  
  tibble_2019 <- data_2019 |> 
    dplyr::as_tibble() |>
    dplyr::rename(prop_pop_1km_2019 = prop_pop_1km,
                  prop_pop_2km_2019 = prop_pop_2km) |> 
    dplyr::select(-c(Names, geometry, prop_pop_3km, fid,
                     DN, X, dr_2017, dr_2018, dr_2020, SMR))
  
  tibble_2020 <- data_2020 |> 
    dplyr::as_tibble() |>
    dplyr::rename(prop_pop_1km_2020 = prop_pop_1km,
                  prop_pop_2km_2020 = prop_pop_2km) |> 
    dplyr::select(-c(Names, geometry, prop_pop_3km, fid,
                     DN, X, dr_2017, dr_2018, dr_2019, SMR))
  
  merged_data <- merge(
  merge(
    merge(
      tibble_2017, tibble_2018, by = "rowID", all = TRUE),
      tibble_2019, by = "rowID", all = TRUE),
      tibble_2020, by = "rowID", all = TRUE)
  ...
  model_data <- merged_data |> 
    dplyr::rename(`2017` = observed_2017,
                  ...
                  `2019` = observed_2019,
                  `2020` = observed_2020) |> 
    tidyr::pivot_longer(cols = c(`2017`,`2018`, `2019`,`2020`),
                        names_to = 'year',
                        values_to = 'malaria_cases') |> 
    dplyr::select(rowID, Names, expected_2017, prop_pop_1km_2017, 
                  prop_pop_2km_2017, expected_2018, prop_pop_1km_2018, 
                  prop_pop_2km_2018, expected_2019, prop_pop_1km_2019,
                  prop_pop_2km_2019, expected_2020, prop_pop_1km_2020,
                  prop_pop_2km_2020, year, malaria_cases)
...
# Invoking function ------------------------------------------------------------
model_data <- merge.model.data(model_data_2017, model_data_2018, 
                               model_data_2019, model_data_2020) 

dplyr::glimpse(model_data)


# Model fitting ----------------------------------------------------------------
multivariate_1km_2017 <- glm(malaria_cases~1+prop_pop_1km_2017+year+offset(log(expected_2017)),
...
multivariate_2km_2017 <- glm(malaria_cases~1+prop_pop_2km_2017+year+offset(log(expected_2017)),
                             data = model_data, family = 'poisson')
...
multivariate_1km_2018 <- glm(malaria_cases~1+prop_pop_1km_2018+year+offset(log(expected_2018)),
                             data = model_data, family = 'poisson')

summary(multivariate_1km_2018)
sjPlot::tab_model(multivariate_1km_2018)

multivariate_2km_2018 <- glm(malaria_cases~1+prop_pop_2km_2018+year+offset(log(expected_2018)),
                             data = model_data, family = 'poisson')

summary(multivariate_2km_2018)
sjPlot::tab_model(multivariate_2km_2018)

multivariate_1km_2019 <- glm(malaria_cases~1+prop_pop_1km_2019+year+offset(log(expected_2019)),
                             data = model_data, family = 'poisson')

summary(multivariate_1km_2019)
sjPlot::tab_model(multivariate_1km_2019)

multivariate_2km_2019 <- glm(malaria_cases~1+prop_pop_2km_2019+year+offset(log(expected_2019)),
                             data = model_data, family = 'poisson')

summary(multivariate_2km_2019)
sjPlot::tab_model(multivariate_2km_2019)


multivariate_1km_2020 <- glm(malaria_cases~1+prop_pop_1km_2020+year+offset(log(expected_2020)),
                             data = model_data, family = 'poisson')

summary(multivariate_1km_2020)
sjPlot::tab_model(multivariate_1km_2020)

multivariate_2km_2020 <- glm(malaria_cases~1+prop_pop_2km_2020+year+offset(log(expected_2020)),
                             data = model_data, family = 'poisson')

summary(multivariate_2km_2020)
sjPlot::tab_model(multivariate_2km_2020)
