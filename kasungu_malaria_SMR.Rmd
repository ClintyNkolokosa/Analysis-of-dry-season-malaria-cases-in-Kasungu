...
# Model fitting ----------------------------------------------------------------
# Get estimated coefficients: intercept and slope
jFun <- function(x) {
  estCoefs <- coef(glm(observed_cases~1+prop_pop_1km+year+offset(log(expected_cases)),
                       family = 'poisson', x))
  names(estCoefs) <- c("intercept", "slope")
  return(estCoefs)
}

jCoefs <- plyr::ddply(model_data,observed_cases~1+prop_pop_1km+year+offset(log(expected_cases)), jFun)
