...
In a `quassi-Poisson` model, the `standard errors` are inflated by multiplying the `variance` by $ϕ$, so that the `standard errors` are larger than the likelihood approach would imply; i.e., $SEQ(β) = √ϕ∗SE(β)$, where $Q$ stands for “quasi-Poisson” since multiplying variances by $ϕ$ is an ad-hoc solution (Roback and Legler, 2021).
