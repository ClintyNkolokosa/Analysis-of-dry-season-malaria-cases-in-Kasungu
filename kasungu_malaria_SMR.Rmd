...
train_model_1km <- caret::train(observed_cases~1+prop_pop_1km+year+offset(log(expected_cases)),
                                data = model_data_evaluation, method = "glm", trControl = train_control)

train_model_2km <- caret::train(observed_cases~1+prop_pop_2km+year+offset(log(expected_cases)),
                                data = model_data_evaluation, method = "glm", trControl = train_control)


train_model_3km <- caret::train(observed_cases~1+prop_pop_3km+year+offset(log(expected_cases)),
                                data = model_data_evaluation, method = "glm", trControl = train_control)
...
```{r, message=FALSE, warning=FALSE}
separated_coord <- zipatala_aggregated_sf |>
  as.data.frame() |>
  dplyr::mutate(long = unlist(map(zipatala_aggregated_sf$geometry, 1)),
                lat = unlist(map(zipatala_aggregated_sf$geometry, 2)))

# Function to split an scf_POINT object with single geometry columns to 
# a columns with separate longitude and latitude columns
