...
# Calculate confidence intervals for dry season malaria cases
```{r message = FALSE, warning=FALSE, fig.height=5, fig.width=6}
# Get CI from binomial distribution and malaria proportion ---------------------
malaria_CI_2017 <- SMR_2017 |>
  dplyr::mutate(epitools::binom.exact(
    observed_2017, pop_2017)) |>
  dplyr::mutate(
    malaria_prop = round(observed_2017/ sum(observed_2017), 2))

malaria_CI_2018 <- SMR_2018 |>
  dplyr::mutate(epitools::binom.exact(
    observed_2018, pop_2018)) |>
  dplyr::mutate(
    malaria_prop = round(observed_2018/ sum(observed_2018), 2))

malaria_CI_2019 <-SMR_2019  |>
  dplyr::mutate(epitools::binom.exact(
    observed_2019, pop_2019)) |>
  dplyr::mutate(
    malaria_prop = round(observed_2019/ sum(observed_2019), 2))

malaria_CI_2020 <- SMR_2020 |>
  dplyr::mutate(epitools::binom.exact(
    observed_2020, pop_2020)) |>
  dplyr::mutate(
    malaria_prop = round(observed_2020/ sum(observed_2020), 2))

# Add Kasungu dry season malaria rate ------------------------------------------
malaria_CI_2017$overall_rate <- overall_malaria_incidence_rate_2017

malaria_CI_2018$overall_rate <- overall_malaria_incidence_rate_2018

malaria_CI_2019$overall_rate <- overall_malaria_incidence_rate_2019

malaria_CI_2020$overall_rate <- overall_malaria_incidence_rate_2020

# Subset the high SMR data -----------------------------------------------------
SMR_high_2017 <- malaria_CI_2017[malaria_CI_2017$SMR > 1, ]

SMR_high_2018 <- malaria_CI_2018[malaria_CI_2018$SMR > 1, ]

SMR_high_2019 <- malaria_CI_2019[malaria_CI_2019$SMR > 1, ]

SMR_high_2020 <- malaria_CI_2020[malaria_CI_2020$SMR > 1, ]

# Plot estimates with CIs ------------------------------------------------------
ggplot2::ggplot(SMR_high_2017, aes(x = Names, 
                                   y = malaria_prop / overall_rate,
                                   ymin = lower / overall_rate, 
                                   ymax = upper / overall_rate)) +
        geom_pointrange() +
...
# Check probability that a value is over a threshold. 
binom.exceed <- function(observed, population, expected, e){
  
    1 - pbinom(e * expected, population, prob = observed / population)
}

# Compute probability of a doubling SMR i.e., P(rate > 2)
malaria_CI_2017$prob_exceedance <- binom.exceed(observed = malaria_CI_2017$observed_2017,
                                                population = malaria_CI_2017$pop_2017,
                                                expected = malaria_CI_2017$expected_2017,
                                                e = 2)


# Convert to spatial object
malaria_CI_2017_sp <- as(malaria_CI_2017, "Spatial")

# Plot the P(rate > 2) map
mapview(malaria_CI_2017_sp, zcol = "prob_exceedance")
...
moran.test(spatial_dependency_data$observed_2017, 
...
zipatala <- health_facility_aggr_sf |> 
