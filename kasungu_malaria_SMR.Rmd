...
# sf::st_write(buffer_1km_2017, "data/buffer_1km_2017.shp")

buffer_2km_2017 <- create.waterbody.buffer(waterbody = surface_waterbodies_2017, 
                                           distance = 2000, 
                                           catchment = malire_new)

# sf::st_write(buffer_2km_2017, "data/buffer_2km_2017.shp")

buffer_3km_2017 <- create.waterbody.buffer(waterbody = surface_waterbodies_2017, 
                                           distance = 3000,
                                           catchment = malire_new)

# sf::st_write(buffer_3km_2017, "data/buffer_3km_2017.shp")
...
# sf::st_write(buffer_1km_2018, "data/buffer_1km_2018.shp")

buffer_2km_2018 <- create.waterbody.buffer(waterbody = surface_waterbodies_2018, 
                                           distance = 2000, 
                                           catchment = malire_new)

# sf::st_write(buffer_2km_2018, "data/buffer_2km_2018.shp")

buffer_3km_2018 <- create.waterbody.buffer(waterbody = surface_waterbodies_2018, 
                                           distance = 3000, 
                                           catchment = malire_new)

# sf::st_write(buffer_3km_2018, "data/buffer_3km_2018.shp")
...
# sf::st_write(buffer_1km_2019, "data/buffer_1km_2019.shp")

buffer_2km_2019 <- create.waterbody.buffer(waterbody = surface_waterbodies_2019, 
                                           distance = 2000, 
                                           catchment = malire_new)

# sf::st_write(buffer_2km_2019, "data/buffer_2km_2019.shp")

buffer_3km_2019 <- create.waterbody.buffer(waterbody = surface_waterbodies_2019, 
                                           distance = 3000, 
                                           catchment = malire_new)

# sf::st_write(buffer_3km_2019, "data/buffer_3km_2019.shp")
...
# sf::st_write(buffer_1km_2020, "data/buffer_1km_2020.shp")

buffer_2km_2020 <- create.waterbody.buffer(waterbody = surface_waterbodies_2020, 
                                           distance = 2000, 
                                           catchment = malire_new)

# sf::st_write(buffer_1km_2020, "data/buffer_2km_2020.shp")

buffer_3km_2020 <- create.waterbody.buffer(waterbody = surface_waterbodies_2020, 
                                           distance = 3000, 
                                           catchment = malire_new)

# sf::st_write(buffer_1km_2020, "data/buffer_3km_2020.shp")
...
Using this formula, for each change in number of people living near dams, we can predict the number of dry season malaria case.
Our estimated model:
       *dry season malaria risk = -0.29 + 0.03 buffer population + ln(e)*
If the number of people living around dams was 0, dry season malaria risk would be -0.29. That is the estimate 0.29 can be interpreted as the dry season malaria risk in a catchment with no people living near waterbodies.

For example:

   - for a buffer population equal zero, we can expect:
          = -0.29 malaria cases ±2 x standard error
          = -0.29 - 2 x 0.0006191 = -0.2912382 malaria cases or
          = -0.29 + 2 x 0.0006191 = -0.2887618 malaria cases, with a 
...
```
In this combined dataset, we can see that the `residual deviance` is far from `degrees of freedom`, and the `dispersion parameter` is e.g., 576.9798 (57121/99 ) which is large.
What does this indicate? This value indicates poor fit. That is, a significant difference between fitted values and observed values.
This means that there is extra variance not accounted for by the model. One way to deal with over-dispersion is to run a `quasi-Poisson` model, which fits an extra dispersion parameter to account for that extra variance.
The negative coefficient for the predictors is highly significant (`p<2e-16`).
We also see that yearly variation influences observed cases negatively, but percentage of people living close to dams influences observed cases positively. 


```{r, message=FALSE, fig.height=17, fig.width=16}
# Check effect of removing intercept
