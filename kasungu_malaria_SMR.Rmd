...
The `expected` number of dry season malaria cases in catchment *i* are calculated as the observed risk (r) of malaria i.e. the total number of malaria cases in the catchment area divided by the total population of Kasungu district, multiplied by the number of people in that catchment area: $$E_i = \frac{\sum_i O_i}{\sum_i N_i}\times N_i$$

Expected number of dry season malaria cases are calculated under the assumption that there is no spatial variation in risk, i.e., no difference in infection rates between the catchment areas.
...
We do this by comparing what we `observe` (O) with what we'd `expect` (E) if the risk of malaria was equal throughout Kasungu. The SMR statistical notation of catchment *i* can be written as follows:
$$SMR_i = \frac{O_i}{E_i}$$
...
```{r, message=FALSE, warning=FALSE, fig.height=15, fig.width=11, fig.cap='Fig 9. Relation between standardised morbidity ratio and living near waterbodies'}
