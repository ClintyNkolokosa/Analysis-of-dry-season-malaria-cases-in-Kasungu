...
df2017 <- model_data_2017 |> 
  dplyr::as_tibble() |> 
    ...
    dplyr::select(-geometry, -prop_pop_3km, -fid,
                  -DN, -X, -SMR, -pop_2017,
...
# Model fitting ----------------------------------------------------------------
multivariate_1km <- glm(observed_cases~1+prop_pop_1km+factor(year)+offset(log(expected_cases)),
...
multivariate_2km <- glm(observed_cases~1+prop_pop_2km+factor(year)+offset(log(expected_cases)),
                        data = model_data, family = 'poisson')
