...
dry_season_malaria_cases <- read.csv("KasunguData.csv")

# Kasungu district boundary shapefile 
kasungu_district <- st_read("kasungu_district.shp")
  
# Kasungu health facility catchment boundary shapefile 
malire <- st_read("kasungu_health_facility_catchments.shp") 

# Kasungu population raster layer
kasungu_population_2017 <- raster("ku_pop_2017_1km_aggregated.tif")
kasungu_population_2018 <- raster("ku_pop_2018_1km_aggregated.tif")
kasungu_population_2019 <- raster("ku_pop_2019_1km_aggregated.tif")
kasungu_population_2020 <- raster("ku_pop_2020_1km_aggregated.tif")

# Open waterbodies polygons 
water_2017 <- st_read("water_bodies_2017.shp")
water_2018 <- st_read("kasungu_2018_water.shp")
water_2019 <- st_read("kasungu_2019_water.shp")
water_2020 <- st_read("water_bodies_2020.shp")
...
```{r}
st_crs(water_2017) <- crs(water_2018) # CRS was missing for 2017
...
### Commented lines are producing errors: 2) "Error in h(simpleError(msg, call)) : error in evaluating the argument 'x' in selecting a method for function 'addAttrToGeom': conversion from feature type sfc_GEOMETRY to sp is not supported" ###
...
### I'm yet to work on the following chunks as it appears that some arguments e.g. water_int_sf, are missing
