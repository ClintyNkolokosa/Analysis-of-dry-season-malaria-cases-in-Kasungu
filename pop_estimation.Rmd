...
create.choropleth.map <- function(df, vname = NA, pal = NA, legend.title = NA){
  # choropleth map
...
population <- create.choropleth.map(malire, vname = "POPULATION", 
                                    pal = "Reds", legend.title = "Total population")

pop_density <- create.choropleth.map(malire, vname = "DENSITY", 
                                     pal = "YlOrRd", legend.title = "Population density")

malaria_rate <- create.choropleth.map(malire, vname = "MALAR_RATE", 
                                      pal = "BuGn", legend.title = "Malaria prevalence %")

malaria_cases <- create.choropleth.map(malire, vname = "MALARIA_CA", 
...
create.population.map <- function(df, title){
  # population map
  # arguments:
...
pop_2017 <- create.population.map(kasungu_population_2017, title = "2017 Population")

pop_2018 <- create.population.map(kasungu_population_2018, title = "2018 Population")

pop_2019 <- create.population.map(kasungu_population_2019, title = "2019 Population")

pop_2020 <- create.population.map(kasungu_population_2020, title = "2020 Population")
...
buffer_1km_2017 <- create.waterbody.buffer(waterbody = surfaceWater_2017, 
                                           distance = 1000, catchment = malire_new)

buffer_2km_2017 <- create.waterbody.buffer(waterbody = surfaceWater_2017, 
                                           distance = 2000, catchment = malire_new)

buffer_3km_2017 <- create.waterbody.buffer(waterbody = surfaceWater_2017, 
                                           distance = 3000, catchment = malire_new)

# For 2018 TropWet surface water polygons
buffer_1km_2018 <- create.waterbody.buffer(waterbody = surfaceWater_2018, 
                                           distance = 1000, catchment = malire)

buffer_2km_2018 <- create.waterbody.buffer(waterbody = surfaceWater_2018, 
                                           distance = 2000, catchment = malire)

buffer_3km_2018 <- create.waterbody.buffer(waterbody = surfaceWater_2018, 
                                           distance = 3000, catchment = malire)
 
# For 2019 TropWet surface water polygons
buffer_1km_2019 <- create.waterbody.buffer(waterbody = surfaceWater_2019, 
                                           distance = 1000, catchment = malire)

buffer_2km_2019 <- create.waterbody.buffer(waterbody = surfaceWater_2019, 
                                           distance = 2000, catchment = malire)

buffer_3km_2019 <- create.waterbody.buffer(waterbody = surfaceWater_2019, 
                                           distance = 3000, catchment = malire)

# For 2020 TropWet surface water polygons
buffer_1km_2020 <- create.waterbody.buffer(waterbody = surfaceWater_2020, 
                                           distance = 1000, catchment = malire)

buffer_2km_2020 <- create.waterbody.buffer(waterbody = surfaceWater_2020, 
                                           distance = 2000, catchment = malire)

buffer_3km_2020 <- create.waterbody.buffer(waterbody = surfaceWater_2020, 
                                           distance = 3000, catchment = malire)
...
# Map the buffers
create.buffer.map <- function(df, boundary, title = NA){
...
# For 2017 
buffer1km_2017 <- create.buffer.map(buffer_1km_2017, kasungu_district, title = "2017: 1km Buffers")
buffer2km_2017 <- create.buffer.map(buffer_2km_2017, kasungu_district, title = "2017: 2km Buffers")
buffer3km_2017 <- create.buffer.map(buffer_3km_2017, kasungu_district, title = "2017: 3km Buffers")

# For 2018
buffer1km_2018 <- create.buffer.map(buffer_1km_2018, kasungu_district, title = "2018: 1km Buffers")
buffer2km_2018 <- create.buffer.map(buffer_2km_2018, kasungu_district, title = "2018: 2km Buffers")
buffer3km_2018 <- create.buffer.map(buffer_3km_2018, kasungu_district, title = "2018: 3km Buffers")

# For 2019
buffer1km_2019 <- create.buffer.map(buffer_1km_2019, kasungu_district, title = "2019: 1km Buffers")
buffer2km_2019 <- create.buffer.map(buffer_2km_2019, kasungu_district, title = "2019: 2km Buffers")
buffer3km_2019 <- create.buffer.map(buffer_3km_2019, kasungu_district, title = "2019: 3km Buffers")

# For 2020
buffer1km_2020 <- create.buffer.map(buffer_1km_2020, kasungu_district, title = "2020: 1km Buffers")
buffer2km_2020 <- create.buffer.map(buffer_2km_2020, kasungu_district, title = "2020: 2km Buffers")
buffer3km_2020 <- create.buffer.map(buffer_3km_2020, kasungu_district, title = "2020: 3km Buffers")
...
create.pop.map <- function(df, vname = "pop_estimate", title, legend.title = NA){
...
run1k_2017_map <- create.pop.map(run1k_2017, title = "2017: 1km buffers", 
                               legend.title = "Estimated \n population")

run2k_2017_map <- create.pop.map(run2k_2017, title = "2017: 2km buffers", 
                               legend.title = "Estimated \n population")

run3k_2017_map <- create.pop.map(run3k_2017, title = "2017: 3km buffers", 
                               legend.title = "Estimated \n population")

run1k_2018_map <- create.pop.map(run1k_2018, title = "2018: 1km buffers", 
                               legend.title = "Estimated \n population")

run2k_2018_map <- create.pop.map(run2k_2018, title = "2018: 2km buffers", 
                               legend.title = "Estimated \n population")

run3k_2018_map <- create.pop.map(run3k_2018, title = "2018: 3km buffers", 
                               legend.title = "Estimated \n population")

run1k_2019_map <- create.pop.map(run1k_2019, title = "2019: 1km buffers", 
                               legend.title = "Estimated \n population")

run2k_2019_map <- create.pop.map(run2k_2019, title = "2019: 2km buffers", 
                               legend.title = "Estimated \n population")

run3k_2019_map <- create.pop.map(run3k_2019, title = "2019: 3km buffers", 
                               legend.title = "Estimated \n population")

run1k_2020_map <- create.pop.map(run1k_2020, title = "2020: 1km buffers", 
                               legend.title = "Estimated \n population")

run2k_2020_map <- create.pop.map(run2k_2020, title = "2020: 2km buffers", 
                               legend.title = "Estimated \n population")

run3k_2020_map <- create.pop.map(run3k_2020, title = "2020: 3km buffers", 
                               legend.title = "Estimated \n population")
...
make.malaria.map <- function(df, year = NA, title, legend.title = NA){
...
dry_season_malaria_2017 <- make.malaria.map(hfc_1km_2017_sf, year = "dr_2017", title = "2017", 
                                            legend.title = "2017 malaria cases")

dry_season_malaria_2018 <- make.malaria.map(hfc_1km_2018_sf, year = "dr_2018", title = "2018",
                                            legend.title = "2018 malaria cases")

dry_season_malaria_2019 <- make.malaria.map(hfc_1km_2019_sf, year = "dr_2019", title = "2019",
                                            legend.title = "2019 malaria cases")

dry_season_malaria_2020 <- make.malaria.map(hfc_1km_2020_sf, year = "dr_2020", title = "2020",
...
Now, we combine all the dataframes with estimated population and malaria cases into one dataframe `kasungu_model_df`, and new columns (`totalMalaria_year`, `totalPop_year`, `malariaProp_year`, and `expectedMalaria_year`) are then added. The aim is to determine the standardised mortality/morbidity ratio (SMR). SMR compares the risk of morbidity/mortality in a population of interest with that of a standard population. In this case, interest in understanding whether the number of dry season malaria cases in each health facility catchment are greater than we would expect given the rate of malaria in the whole of Kasungu district.Health facility catchments with SMR values less than one have lower than expected dry season malaria cases, whereas those greater than one have higher than expected.
```{r, message=FALSE, warning=FALSE}

# 2017 dataframes --------------------------------------------------------------
hfc_1km_2017_df <- hfc_1km_2017_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(Names, FID, pop_1km_2017,
                dr_2017, dr_2018, dr_2019, dr_2020, geometry)
 
hfc_2km_2017_df <- hfc_2km_2017_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_2km_2017)

hfc_3km_2017_df <- hfc_3km_2017_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_3km_2017)

# 2018 dataframes --------------------------------------------------------------
hfc_1km_2018_df <- hfc_1km_2018_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_1km_2018)

hfc_2km_2018_df <- hfc_2km_2018_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_2km_2018)

hfc_3km_2018_df <- hfc_3km_2018_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_3km_2018)

# 2019 dataframes --------------------------------------------------------------
hfc_1km_2019_df <- hfc_1km_2019_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_1km_2019)

hfc_2km_2019_df <- hfc_2km_2019_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_2km_2019)

hfc_3km_2019_df <- hfc_3km_2019_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_3km_2019)

# 2020 dataframes --------------------------------------------------------------
hfc_1km_2020_df <- hfc_1km_2020_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_1km_2020)

hfc_2km_2020_df <- hfc_2km_2020_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_2km_2020)

hfc_3km_2020_df <- hfc_3km_2020_sf %>% 
  unclass %>% 
  dplyr::as_tibble() %>% 
  dplyr::select(pop_3km_2020)

# Bind the dataframes and calculate expected number of dry season malaria cases and SMR
...
kasungu_model_df <- cbind(hfc_1km_2017_df, hfc_2km_2017_df, hfc_3km_2017_df,
  ...
  dplyr::mutate(expectedMalaria_1km_2017 = round(sum(dr_2017)/sum(pop_1km_2017, na.rm = TRUE)*pop_1km_2017),
                expectedMalaria_2km_2017 = round(sum(dr_2017)/sum(pop_2km_2017, na.rm = TRUE)*pop_2km_2017),
                expectedMalaria_3km_2017 = round(sum(dr_2017)/sum(pop_3km_2017, na.rm = TRUE)*pop_3km_2017)) %>% 
  
  dplyr::mutate(expectedMalaria_1km_2018 = round(sum(dr_2018)/sum(pop_1km_2018, na.rm = TRUE)*pop_1km_2018),
                expectedMalaria_2km_2018 = round(sum(dr_2018)/sum(pop_2km_2018, na.rm = TRUE)*pop_2km_2018),
                expectedMalaria_3km_2018 = round(sum(dr_2018)/sum(pop_3km_2018, na.rm = TRUE)*pop_3km_2018)) %>% 
  
  dplyr::mutate(expectedMalaria_1km_2019 = round(sum(dr_2019)/sum(pop_1km_2019, na.rm = TRUE)*pop_1km_2019),
                expectedMalaria_2km_2019 = round(sum(dr_2019)/sum(pop_2km_2019, na.rm = TRUE)*pop_2km_2019),
                expectedMalaria_3km_2019 = round(sum(dr_2019)/sum(pop_3km_2019, na.rm = TRUE)*pop_3km_2019)) %>% 
  
  dplyr::mutate(expectedMalaria_1km_2020 = round(sum(dr_2020)/sum(pop_1km_2020, na.rm = TRUE)*pop_1km_2020),
                expectedMalaria_2km_2020 = round(sum(dr_2020)/sum(pop_2km_2020, na.rm = TRUE)*pop_2km_2020),
                expectedMalaria_3km_2020 = round(sum(dr_2020)/sum(pop_3km_2020, na.rm = TRUE)*pop_3km_2020)) %>% 
  
  dplyr::mutate(SMR_1km_2017 = round(dr_2017/expectedMalaria_1km_2017,2),
                SMR_2km_2017 = round(dr_2017/expectedMalaria_2km_2017,2),
                SMR_3km_2017 = round(dr_2017/expectedMalaria_3km_2017,2)) %>% 
  
  dplyr::mutate(SMR_1km_2018 = round(dr_2018/expectedMalaria_1km_2018,2),
                SMR_2km_2018 = round(dr_2018/expectedMalaria_2km_2018,2),
                SMR_3km_2018 = round(dr_2018/expectedMalaria_3km_2018,2)) %>% 
  
  dplyr::mutate(SMR_1km_2019 = round(dr_2019/expectedMalaria_1km_2019,2),
                SMR_2km_2019 = round(dr_2019/expectedMalaria_2km_2019,2),
                SMR_3km_2019 = round(dr_2019/expectedMalaria_3km_2019,2)) %>% 
  
  dplyr::mutate(SMR_1km_2020 = round(dr_2020/expectedMalaria_1km_2020,2),
                SMR_2km_2020 = round(dr_2020/expectedMalaria_2km_2020,2),
                SMR_3km_2020 = round(dr_2020/expectedMalaria_3km_2020,2)) %>% 
  
  dplyr::mutate(malariaProp_1km_2017 = round(sum(pop_1km_2017, na.rm = TRUE)/sum(dr_2017),1),
                malariaProp_2km_2017 = round(sum(pop_2km_2017, na.rm = TRUE)/sum(dr_2017),1),
                malariaProp_3km_2017 = round(sum(pop_3km_2017, na.rm = TRUE)/sum(dr_2017),1)) %>% 
  
  dplyr::mutate(malariaProp_1km_2018 = round(sum(pop_1km_2018, na.rm = TRUE)/sum(dr_2018),1),
                malariaProp_2km_2018 = round(sum(pop_2km_2018, na.rm = TRUE)/sum(dr_2018),1),
                malariaProp_3km_2018 = round(sum(pop_3km_2018, na.rm = TRUE)/sum(dr_2018),1)) %>% 
  
  dplyr::mutate(malariaProp_1km_2019 = round(sum(pop_1km_2019, na.rm = TRUE)/sum(dr_2019),1),
                malariaProp_2km_2019 = round(sum(pop_2km_2019, na.rm = TRUE)/sum(dr_2019),1),
                malariaProp_3km_2019 = round(sum(pop_3km_2019, na.rm = TRUE)/sum(dr_2019),1)) %>% 
  
  dplyr::mutate(malariaProp_1km_2020 = round(sum(pop_1km_2020, na.rm = TRUE)/sum(dr_2020),1),
                malariaProp_2km_2020 = round(sum(pop_2km_2020, na.rm = TRUE)/sum(dr_2020),1),
                malariaProp_3km_2020 = round(sum(pop_3km_2020, na.rm = TRUE)/sum(dr_2020),1))



# Take a glimpse at the model data
table <- kasungu_model_df %>%
  dplyr::select(Names, observed_2017 = `dr_2017`, observed_2018 = `dr_2018`, 
                ...
                SMR_1km_2017, SMR_2km_2017, SMR_3km_2017,
                SMR_1km_2018, SMR_2km_2018, SMR_3km_2018,
                SMR_1km_2019, SMR_2km_2019, SMR_3km_2019,
                SMR_1km_2020, SMR_2km_2020, SMR_3km_2020) %>% 
  kable %>%
...
Let us find out if living within various distances from water bodies is causing variability in SMR in Kasungu district. We hypothesize that the risk of being a case in catchment *i* is dependent on proximity to water bodies. We will model the observed outcomes using `Poisson regression` with an offset of the log of the expected counts `expectedMalaria`. An offset is 'risk factor' term in the model that has a coefficient fixed at 1. `Poisson regression` framework has been used as it is suitable for modeling count outcomes. 
...
# 2017 -----------------------------------------------------------------------------------
model_1km_2017 <- glm(dr_2017~1+malariaProp_1km_2017+offset(log(expectedMalaria_1km_2017)),
...
map <- mapview::mapview(pop_malaria_2017_2020, zcol = "fitted_1km2019")

mapshot(map, "map.html", selfcontained = FALSE)

tm_shape(pop_malaria_2017_2020)+
  tm_polygons(col = "fitted_1km2019",
              style="jenks",
              palette ="Blues", 
              contrast =.7, 
              title = "Fitted values")
